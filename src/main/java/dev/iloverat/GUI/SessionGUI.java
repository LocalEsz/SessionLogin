package dev.iloverat.GUI;

import dev.iloverat.ssidlogin;
import dev.iloverat.Utils.APIUtils;
import dev.iloverat.Utils.SessionChanger;
...
public class SessionGUI extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 1400) {
            new Thread (() -> {
                try {
                    ...
                    // If session is invalid, try with Microsoft token
                    String[] playerInfo = APIUtils.validateSession(token) ?
                        APIUtils.getProfileInfo(token) : APIUtils.getProfileInfo(APIUtils.tokenFromMicrosoft(token));
        ...
        if (button.id == 1500) {
            SessionChanger.setSession(ssidlogin.originalSession);
